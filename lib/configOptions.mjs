...
export const CONFIG_OPTIONS = {
  ...
  // `watch` also accepts the sentinel 'auto' (model-aware: 5h + weekly + any per-model weekly window in use).
  watch: { projectKey: 'watch', globalKey: 'watch', type: 'list', choices: ['five_hour', 'seven_day', 'seven_day_opus', 'seven_day_sonnet'] },
...
export function coerceOptionValue(name, raw) {
  ...
  if (def.type === 'list') {
    if (s.toLowerCase() === 'auto') return 'auto'; // model-aware mode (watch only)
    const seen = new Set();
    const list = [];
