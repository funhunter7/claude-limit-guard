...
export function formatLimit(label, limit, threshold, now = new Date(), locale = undefined, hour12 = false, glyphs = GLYPHS.emoji, warnBand = WARN_BAND) {
...
export function formatStatusLine(usage, threshold, watch, now = new Date(), locale = undefined, hour12 = false, glyphs = GLYPHS.emoji, warnBand = WARN_BAND, labelStyle = 'full') {
  ...
  for (const key of watch) {
    if (labels[key]) parts.push(formatLimit(labels[key], usage[key], threshold, now, locale, hour12, glyphs, warnBand));
  }
