...
export function formatReset(resetsAt, opts = {}) {
  const { now = new Date(), locale = undefined, hour12 = false, glyphs = GLYPHS.emoji, resetDisplay = 'clock' } = opts;
  if (!resetsAt) return '';
...
export function formatLimit(label, limit, threshold, opts = {}) {
  const { now = new Date(), locale = undefined, hour12 = false, glyphs = GLYPHS.emoji, warnBand = WARN_BAND, resetDisplay = 'clock' } = opts;
...
export function formatStatusLine(usage, threshold, watch, opts = {}) {
  const { now = new Date(), locale = undefined, hour12 = false, glyphs = GLYPHS.emoji, warnBand = WARN_BAND, labelStyle = 'full', resetDisplay = 'clock', thresholdOverrides = {}, projectionDisplay = 'off', projection = null } = opts;
  ...
  for (const key of watch) {
    if (labels[key]) {
      ...
      parts.push(formatLimit(labels[key], usage[key], effThreshold, { now, locale, hour12, glyphs, warnBand, resetDisplay }));
    }
  }
