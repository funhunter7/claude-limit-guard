...
// five_hour/seven_day are documented. seven_day_opus/seven_day_sonnet appear in some local
// payloads but are undocumented — mapped best-effort only when present; absent ones are simply
// omitted, so the default watch (five_hour, seven_day) is unaffected.
...
export function coversWatched(stdinUsage, watch) {
  ...
  return watch.every((k) => {
    const win = stdinUsage[k];
    return win != null && typeof win.utilization === 'number' && Number.isFinite(win.utilization);
  });
}

/**
 * True iff any watched window's reset time is at or before `nowMs` — i.e. the window has
 * already rolled over, so the reading (from stdin or cache) is stale and must not be trusted.
 * A window without a `resets_at` is skipped (we cannot tell). Pure: no I/O, no Date.now().
 * @param {object|null} usage Internal usage shape ({ five_hour: { resets_at }, ... }).
 * @param {string[]} watch Watched window keys.
 * @param {number} nowMs Current time in epoch ms.
 * @returns {boolean}
 */
export function watchedExpired(usage, watch, nowMs) {
  if (!usage) return false;
  for (const key of watch || []) {
    const r = usage[key]?.resets_at;
    if (r == null) continue;
    // resets_at is ms (number, from usageFromRateLimits) or an ISO string (passed through).
    const t = typeof r === 'number' ? r : new Date(r).getTime();
