...
export default {
  ...
  notifyReset: (window) => `Your ${window} limit has reset — you're good to go.`,
  // /limit-guard-snooze strings.
  snoozeSet: (u) => `Guard snoozed until ${u}.`,
  snoozeCleared: 'Snooze cleared.',
  snoozeNone: 'No active snooze.',
  // /limit-guard-doctor strings.
  doctorHeader: 'claude-limit-guard doctor',
  doctor_node: 'Node ≥ 18',
  doctor_token: 'OAuth token',
  doctor_statusline: 'status line wired',
  doctor_cache: 'usage cache fresh',
  doctor_version: 'plugin version',
  doctorHint_node: 'Upgrade Node to v18 or newer.',
  doctorHint_token: 'Sign in to Claude Code.',
  doctorHint_statusline: 'Run /limit-guard-setup to wire the status line.',
  doctorHint_cache: 'Open a Claude Code session so the status line refreshes the cache.',
  doctorHint_version: 'Update the plugin with /plugin update.',
  doctorVersionUnknown: 'latest unknown',
  // /limit-guard-setup strings.
  setupWired: 'Status line wired into settings.json.',
  setupAlreadyWired: 'Status line already wired — nothing to do.',
  setupBackedUp: (p) => `Backed up previous settings to ${p}.`,
